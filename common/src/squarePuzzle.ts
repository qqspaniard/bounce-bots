...
export const generateRandomPuzzle = (
  ...
  // Draw center island walls
  _set_wall(cells, xSize / 2 - 1, ySize / 2, 'north');
  _set_wall(cells, xSize / 2 - 1, ySize / 2, 'west');
  _set_wall(cells, xSize / 2 - 1, ySize / 2 - 1, 'west');
  _set_wall(cells, xSize / 2 - 1, ySize / 2 - 1, 'south');
  _set_wall(cells, xSize / 2, ySize / 2 - 1, 'south');
  _set_wall(cells, xSize / 2, ySize / 2 - 1, 'east');
  _set_wall(cells, xSize / 2, ySize / 2, 'east');
  _set_wall(cells, xSize / 2, ySize / 2, 'north');
...
const _set_wall = (
  ...
  if (direction === 'north') {
    cells[_coord_to_id(x, y)].neighbors.north = null;
    cells[_coord_to_id(x, y + 1)].neighbors.south = null;
  } else if (direction === 'east') {
    cells[_coord_to_id(x, y)].neighbors.east = null;
    cells[_coord_to_id(x + 1, y)].neighbors.west = null;
